...
retry_scr:
	...
	while (timeout--) {
        ...
        debug("GET switch_status[7]: %.8x\n", switch_status[7]);
		/* The high-speed function is busy.  Try again */
		if (!(__be32_to_cpu(switch_status[7]) & SD_HIGHSPEED_BUSY))
			break;
	}

    debug("GET switch_status[3]: %.8x\n", switch_status[3]);
	...
	if (err)
		return err;
	debug("GET switch_status[4]: %.8x\n", __be32_to_cpu(switch_status[4]));
...
static int mmc_startup(struct mmc *mmc)
{
	...
	debug("GET CSD: %.8x, %.8x, %.8x, %.8x\n", mmc->csd[0], mmc->csd[1], 
	                                    mmc->csd[2], mmc->csd[3]); 

	if (mmc->version == MMC_VERSION_UNKNOWN) {
		int version = (cmd.response[0] >> 26) & 0xf;

        debug("Get MMC version from CSD value: %d\n", version);
		switch (version) {
			case 0:
