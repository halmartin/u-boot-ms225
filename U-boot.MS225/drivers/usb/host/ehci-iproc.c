...
#define USB_IN_DEVICE_MODE() ((ehci_iol_r(IPROC_WRAP_TOP_STRAP_STATUS) & (1 << 17)) == 0)
...
#define USBH_ENABLE_VBUS(val, gpio)         val &= ~(1 << gpio)
#define USB_IN_DEVICE_MODE() ((ehci_iol_r(IPROC_WRAP_IPROC_STRAP_CTRL) & (1 << 10)) == 0)
...
int linux_usbh_init(void)
{
    ...
    /* enable clock */
    clk_enable = ehci_iol_r(USB2_IDM_IDM_IO_CONTROL_DIRECT);
    printf("Initial usb2h clock is: %08x\n", clk_enable);
    clk_enable |= 1;
    printf("Initial usb2h clock is: %08x\n", clk_enable);
    ehci_iol_w(clk_enable, USB2_IDM_IDM_IO_CONTROL_DIRECT);
...
int usbphy_init(void)
{
    ...
    val = ehci_iol_r(IPROC_WRAP_USBPHY_CTRL_0);
    val |= (1 << IPROC_WRAP_USBPHY_CTRL_0__PHY_ISO);
    ehci_iol_w(val, IPROC_WRAP_USBPHY_CTRL_0);
#else
    val = ehci_iol_r(IPROC_WRAP_USBPHY_CTRL_2);
    val |= (1 << IPROC_WRAP_USBPHY_CTRL_2__PHY_ISO);
    ehci_iol_w(val, IPROC_WRAP_USBPHY_CTRL_2);
#endif
...
#if defined(CONFIG_SABER2)
    val = ehci_iol_r(IPROC_WRAP_USBPHY_CTRL_5);
    val |= (1 << IPROC_WRAP_USBPHY_CTRL_5__P1CTL_R);
    ehci_iol_w(val, IPROC_WRAP_USBPHY_CTRL_5);
#else
    val = ehci_iol_r(IPROC_WRAP_USBPHY_CTRL_2);
    ...
    do {
        val = ehci_iol_r(IPROC_WRAP_MISC_STATUS);
        if ((val & mask) == mask) {
            break;
        } else {
            udelay(10);
    ...
    if (count > 10) {
        printf("%s : PLL not lock! IPROC_WRAP_MISC_STATUS = 0x%08lx\n", __FUNCTION__, val);
    }
...
int ehci_hcd_init(void)
{
    ...
    val = ehci_iol_r(USB2_IDM_IDM_IO_CONTROL_DIRECT);
    ...
    ehci_iol_w(val, USB2_IDM_IDM_IO_CONTROL_DIRECT);

    debug("Enable usb2h clock. (I/O Ctrl Direct: 0x%08x)\n", 
                        ehci_iol_r(USB2_IDM_IDM_IO_CONTROL_DIRECT));

    /* Bring USB Host out of reset */
    printf("Bring usb2h_out of reset.......\n");
    val = ehci_iol_r(USB2_IDM_IDM_RESET_CONTROL);
    val |= (1 << USB2_IDM_IDM_RESET_CONTROL__RESET);
    ehci_iol_w(val, USB2_IDM_IDM_RESET_CONTROL);
    mdelay(1);
    val &= ~(1 << USB2_IDM_IDM_RESET_CONTROL__RESET);
    ehci_iol_w(val, USB2_IDM_IDM_RESET_CONTROL);

    debug("usb2_reset_state is set and now it is: %08x\n", 
                            ehci_iol_r(USB2_IDM_IDM_RESET_CONTROL));

    val = ehci_iol_r(USBH_Phy_Ctrl_P0);
    ...
    ehci_iol_w(val, USBH_Phy_Ctrl_P0);

    mdelay(100);

    ehci_iol_w(BCM_USB_FIFO_THRESHOLD, USBH_INSNREG01);

    /* Turn on USB module */
    val = ehci_iol_r(ChipcommonG_GP_DATA_OUT);
    USBH_ENABLE_VBUS(val, USBH_VBUS_GPIO);
    ehci_iol_w(val, ChipcommonG_GP_DATA_OUT);
    val = ehci_iol_r(ChipcommonG_GP_OUT_EN);
    val |= (1 << USBH_VBUS_GPIO);
    ehci_iol_w(val, ChipcommonG_GP_OUT_EN);
