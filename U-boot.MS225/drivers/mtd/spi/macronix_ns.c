...
static int set_4byte_mode(struct spi_flash *flash, int enable)
{
	...
	cmd = enable? CMD_MX25XX_EN4B : CMD_MX25XX_EX4B;
	...
	spi_flash_cmd(flash->spi, CMD_MX25XX_WRDI, NULL, 0);
	return 0;
}
...
//	}
	...
	do {
	    ret = spi_flash_cmd(flash->spi, cmd, &status, 1);
...
int macronix_erase(struct spi_flash *flash, u32 offset, size_t len)
{
	...
	for (actual = 0; actual < len; actual++) {
		...
		cmd[1 + mode4b] = (offset / sector_size) + actual;
	...
	debug("SF: Macronix: Successfully erased %lu bytes @ 0x%x\n",
