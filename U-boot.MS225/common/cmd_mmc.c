...
extern int mmc_select_hwpart(int dev_num, int hwpart);
...
static struct mmc *init_mmc_device(int dev, char force_init)
{
	struct mmc *mmc;
...
int do_mmcinfo(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	mmc = init_mmc_device(curr_device, 0);
	if (!mmc)
			 ...
			 int argc, char * const argv[])
{
	...
	mmc = init_mmc_device(curr_device, 1);
	if (!mmc)
		return CMD_RET_FAILURE;
