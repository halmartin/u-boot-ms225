 ...
 * $Id: shmoo_and28.c,v 1.1.1.1 2014/10/09 13:43:34 Exp $
...
#define CHIP_IS_GREYHOUND2          TRUE
#else
#define CHIP_IS_GREYHOUND2          FALSE
#endif
...
#define CHIP_IS_HURRICANE3          TRUE
#else
#define CHIP_IS_HURRICANE3          FALSE
#endif
...
_initialize_bist(int unit, int phy_ndx, int bit, and28_shmoo_container_t *scPtr)
{
    ...
    switch ((*scPtr).shmooType) {
        ...
        case SHMOO_AND28_RD_EXTENDED:
            ...
            bist_info.read_weight = 255;
            bist_info.bist_timer_us = 0;
            bist_info.bist_num_actions = 0x51510;
            bist_info.bist_start_address = 0x00000000;
            bist_info.bist_end_address = 0x00FFFFFF;
            bist_info.mpr_mode = 0;
            bist_info.prbs_mode = 1;
...
_and28_zq_calibration(int unit, int phy_ndx)
{
    ...
    DDR_PHY_REG_READ(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_VREF_DAC_CONTROL, &vref_dac);
    if(shmoo_dram_info.dram_type == SHMOO_AND28_DRAM_TYPE_DDR4)
    {
        data = vref_dac;
    ...
    if(shmoo_dram_info.dram_type == SHMOO_AND28_DRAM_TYPE_DDR4)
    {
        ...
        if(p_drive < 6)
        {
            printf("     WARNING: ZQ calibration error (P offset) - Manual IO programming required for correct operation\n");
            /* return SOC_E_FAIL; */
        }
        else
        {
    ...
    DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, DRIVE_PAD_CTL, ADDR_CTL_PD_STRENGTH, aq_p_drive);
    DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, DRIVE_PAD_CTL, ADDR_CTL_ND_STRENGTH, aq_n_drive);
    DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_DRIVE_PAD_CTL, data);
    #ifdef PHY_AND28_F0
        DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_DRIVE_PAD_CTL_2T, data);
    #endif
    ...
    if(shmoo_dram_info.dram_type == SHMOO_AND28_DRAM_TYPE_DDR4)
    {
        DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_VREF_DAC_CONTROL, vref_dac);
        sal_usleep(SHMOO_AND28_SHORT_SLEEP);
    }
    ...
    printf("     AQ P drive.......: 0x%02lX\n", aq_p_drive);
    printf("     AQ N drive.......: 0x%02lX\n", aq_n_drive);
...
_shmoo_and28_entry(int unit, int phy_ndx, and28_shmoo_container_t *scPtr, uint32 mode)
{
    ...
    /*R04*/ DDR_PHY_REG_READ(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_VREF_DAC_CONTROL, &data);
            ...
            {
                DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, VREF_DAC_CONTROL, DAC0, 32);
                DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, VREF_DAC_CONTROL, DAC1, 44);
            }
            else
            {
                DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, VREF_DAC_CONTROL, DAC0, 32);
    ...
    /*R08*/ if(shmoo_dram_info.sim_system_mode)
            ...
            DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_BYTE_LANE_0, READ_CONTROL, MODE, 0);
            #endif
                ...
                case 1600:
                    if(shmoo_dram_info.dram_type == SHMOO_AND28_DRAM_TYPE_DDR4)
                    {
                        ...
                        switch(shmoo_dram_info.num_bank_groups) 
                        {
                            ...
                            default:
                                printf("Unsupported number of bank groups: %d\n", shmoo_dram_info.num_bank_groups);
                                return SOC_E_FAIL;
                        }
...
/*A09*/ printf("A09. Configure Static Pad Control\n");
        ...
        if(shmoo_dram_info.dram_type == SHMOO_AND28_DRAM_TYPE_DDR4)
        {
            DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, STATIC_PAD_CTL, AUTO_OEB, 0);
        }
        else
        {
            DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, STATIC_PAD_CTL, AUTO_OEB, 1);
        }
        DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, STATIC_PAD_CTL, IDDQ_CS1, 1);
        DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_STATIC_PAD_CTL, data);

        #ifdef SOC_DDR4_SUPPORT
        if(shmoo_dram_info.dram_type == SHMOO_AND28_DRAM_TYPE_DDR4)
        {
            DDR_PHY_REG_READ(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_BYTE_LANE_0_STATIC_PAD_CTL, &data);
            DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_BYTE_LANE_0, STATIC_PAD_CTL, DM_INVERT, 0);
            DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_BYTE_LANE_0_STATIC_PAD_CTL, data);
            DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_BYTE_LANE_1_STATIC_PAD_CTL, data);
            #if(SHMOO_AND28_PHY_BITWIDTH_IS_32)
            if(shmoo_dram_info.interface_bitwidth == 32)
            {
                DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_BYTE_LANE_2_STATIC_PAD_CTL, data);
                DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_BYTE_LANE_3_STATIC_PAD_CTL, data);
            }
            #endif
            DDR_PHY_REG_READ(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_ABI_PAR_CNTRL, &data);
            DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_CONTROL_REGS, ABI_PAR_CNTRL, PAR_ENABLE, 1);
            DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_ABI_PAR_CNTRL, data);
        }
        #endif
        sal_usleep(SHMOO_AND28_SHORT_SLEEP);
...
/*A12*/ printf("A12. Configure Auto Idle\n");
        ...
        if(shmoo_dram_info.interface_bitwidth != 32)
        {
            DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_BYTE_LANE_0, IDLE_PAD_CONTROL, IDLE, 1);
            DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_BYTE_LANE_0, IDLE_PAD_CONTROL, RXENB, 1);
            DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_BYTE_LANE_0, IDLE_PAD_CONTROL, IDDQ, 1);
            #if (defined(PHY_AND28_E0) || defined(PHY_AND28_E2))
                DDR_PHY_SET_FIELD(data, DDR34_CORE_PHY_BYTE_LANE_0, IDLE_PAD_CONTROL, IO_IDLE_ENABLE, 0x7FF);
            #endif
        }
        ...
        #if defined(PHY_AND28_F0)
            DDR_PHY_REG_WRITE(unit, SHMOO_AND28_PHY_REG_BASE, 0, DDR34_CORE_PHY_CONTROL_REGS_STANDBY_CONTROL, 0);
        #endif
        sal_usleep(SHMOO_AND28_SHORT_SLEEP);
