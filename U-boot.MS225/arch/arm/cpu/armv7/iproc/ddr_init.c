...
#if !defined(CONFIG_GREYHOUND2)   /* There is no such bit definition */
	/* de-assert reset */
	__udelay(200);
...
#elif (defined(CONFIG_GREYHOUND) || defined(CONFIG_HURRICANE3) || \
	defined(CONFIG_SABER2) || defined(CONFIG_GREYHOUND2))
...
void ddr_init3(void)
{
    ...
    if (ddr_clk != 800)  /* only DDR3 may run in 667 MHz */
    	  ddr_type = 0;
#endif
    
    ddr_type = (int)val;
    printf("*** DDR type %d: DDR-%s, %s bit mode, speed = %dMHz ***\n", 
          val, (val == 1)? "4" : "3", (is_ddr_32bit() == 1)? "32" : "16", ddr_clk);
    ...
    */
    if (is_ddr_32bit()) {
        reg32_write_masked((volatile uint32_t *)DDR_S2_IDM_IO_CONTROL_DIRECT, 
				    1 << DDR_S2_IDM_IO_CONTROL_DIRECT__mode_32b, 1 << DDR_S2_IDM_IO_CONTROL_DIRECT__mode_32b);
		}
    reg32_write_masked((volatile uint32_t *)DDR_S2_IDM_IO_CONTROL_DIRECT, 0x00000080, 0x00000080);
    val = reg32_read((volatile uint32_t *)DDR_S2_IDM_IO_CONTROL_DIRECT);
    dbg_printf("ReadAfterWrite DDR_S2_IDM_IO_CONTROL_DIRECT(0x%x) Val = 0x%x\n", DDR_S2_IDM_IO_CONTROL_DIRECT, val);
    ...
    */
    ...
    val = reg32_read((volatile uint32_t *)CRU_ddrphy_pwr_ctrl);
    dbg_printf("Read CRU_ddrphy_pwr_ctrl(0x%x) Val = 0x%x\n", CRU_ddrphy_pwr_ctrl, val);
    ...
    */
...
#if defined(CONFIG_GREYHOUND2)
    val = reg32_read((volatile uint32_t *)DDR_S0_IDM_RESET_CONTROL);
    if (val != 0)
    {
    	 reg32_write((volatile uint32_t *)DDR_S0_IDM_RESET_CONTROL, 0);
    	 val = reg32_read((volatile uint32_t *)DDR_S0_IDM_RESET_CONTROL);
       dbg_printf("ReadAfterWrite DDR_S0_IDM_RESET_CONTROL(0x%x) Val = 0x%x\n", DDR_S0_IDM_RESET_CONTROL, val);
    }
#endif
    
    val = reg32_read((volatile uint32_t *)DDR_S1_IDM_RESET_CONTROL);
    if (val != 0)
    {
    	 reg32_write((volatile uint32_t *)DDR_S1_IDM_RESET_CONTROL, 0);
    	 val = reg32_read((volatile uint32_t *)DDR_S1_IDM_RESET_CONTROL);
       dbg_printf("ReadAfterWrite DDR_S1_IDM_RESET_CONTROL(0x%x) Val = 0x%x\n", DDR_S1_IDM_RESET_CONTROL, val);
    }
    
    val = reg32_read((volatile uint32_t *)DDR_S2_IDM_RESET_CONTROL);
    if (val != 0)
    {
    	 reg32_write((volatile uint32_t *)DDR_S2_IDM_RESET_CONTROL, 0);
    	 val = reg32_read((volatile uint32_t *)DDR_S2_IDM_RESET_CONTROL);
       dbg_printf("ReadAfterWrite DDR_S2_IDM_RESET_CONTROL(0x%x) Val = 0x%x\n", DDR_S2_IDM_RESET_CONTROL, val);
    }
    ...
    if(i == 0x19000) {
        printf("DDR PHY not power up(0x%x)\n", DDR_S1_IDM_IO_STATUS);
        return; // Let it go
    } else {
        printf("DDR PHY is power up(0x%x)\n", DDR_S1_IDM_IO_STATUS);
    }

    // Power up DQ/DQS receivers: PDN0~3=0, DAC0~1=32
    val = 0x0001f820;
    dbg_printf("Write DDR_PHY_CONTROL_REGS_VREF_DAC_CONTROL(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_VREF_DAC_CONTROL, val);
    reg32_write_masked((volatile uint32_t *)DDR_PHY_CONTROL_REGS_VREF_DAC_CONTROL, 0xffffffff, val);
    ...
    */
...
#ifdef CONFIG_DDR32
    unsigned int r, bl, maxbl = 4;
#else
    unsigned int r, bl, maxbl = 2;
#endif
    ...
    val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STANDBY_CONTROL);
    dbg_printf("ReadAfterWrite DDR_PHY_CONTROL_REGS_STANDBY_CONTROL(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_STANDBY_CONTROL, val);
    ...
    val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_DFI_CNTRL);
    dbg_printf("ReadAfterWrite DDR_PHY_CONTROL_REGS_DFI_CNTRL(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_DFI_CNTRL, val);
    ...
    dbg_printf("Read DDR_PHY_CONTROL_REGS_DRAM_CONFIG(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_DRAM_CONFIG, val);
    ...
    */
    ...
    if (ddr_type == 0) {   /* DDR3 */
        val |= (1 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__INIT_MODE);
        if (is_ddr_32bit()) {
        	  val &= ~(1 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__BUS16);  /* 32 bit */
        } else {
            val |= (1 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__BUS16);   /* 16 bit */
        }
        //val |= (1 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__GROUP_BITS_R);  
        //val &= ~(1 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__BANK_BITS_R);
        val |= (1 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__BANK_BITS_R);
        val |= (1 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__COL_BITS_R);    
        val |= (3 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__ROW_BITS_R);
        //val |= (1 << DDR_PHY_CONTROL_REGS_DRAM_CONFIG__DRAM_TYPE_R);  /* 0 for DDR3; 1 for DDR4 */
        
        dbg_printf("Write DDR3 DDR_PHY_CONTROL_REGS_DRAM_CONFIG(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_DRAM_CONFIG, val);
        reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_DRAM_CONFIG, val);
        ...
        dbg_printf("Write DDR4 DDR_PHY_CONTROL_REGS_DRAM_CONFIG(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_DRAM_CONFIG, val);
        reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_DRAM_CONFIG, val);
    }
    
    /* PLL setting (not necessary in Emulation) */
#if defined(CONFIG_IPROC_EMULATION)
#else
    val = 0x00810012;
    dbg_printf("Write \"ddr_pllbypass\" DDR_PHY_CONTROL_REGS_PLL_CONFIG Val = 0x%x\n", val);
    reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_PLL_CONFIG, val);

    val = 0x0010307f;   
    dbg_printf("Write \"ddr_pllbypass\" DDR_PHY_CONTROL_REGS_PLL_DIVIDERS Val = 0x%x\n", val);
    reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_PLL_DIVIDERS, val);

    val = 0x000000d0;   
    dbg_printf("Write \"ddr_pllbypass\" DDR_PHY_CONTROL_REGS_PLL_CONTROL1 Val = 0x%x\n", val);
    reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_PLL_CONTROL1, val);

    val = 0x9c000000;   
    dbg_printf("Write \"ddr_pllbypass\" DDR_PHY_CONTROL_REGS_PLL_CONTROL2 Val = 0x%x\n", val);
    reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_PLL_CONTROL2, val);
    ...
    dbg_printf("Read DDR_PHY_CONTROL_REGS_VREF_DAC_CONTROL(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_VREF_DAC_CONTROL, val);
    if (ddr_type == 0) {   /* DDR3 */
        // Do nothing....
    } else {               /* DDR4 */
        ...
        reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_VREF_DAC_CONTROL, val);
    }
    ...
    dbg_printf("Write DDR_PHY_CONTROL_REGS_VDL_CALIBRATE(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_VDL_CALIBRATE, 0x0);
    reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_VDL_CALIBRATE, 0x00000000);

    /* 
     * [4] = 1 [3] = 1 [0] = 1; others are 0
     *
     * SV: Others = 0
     ...
     */
    dbg_printf("Write DDR_PHY_CONTROL_REGS_VDL_CALIBRATE(0x%x) Val = 0x%x\n", DDR_PHY_CONTROL_REGS_VDL_CALIBRATE, 0x19);
    ...
    dbg_printf("Write DDR_PHY_CONTROL_REGS_VDL_CALIBRATE(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_VDL_CALIBRATE, 0x0);
    reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_VDL_CALIBRATE, 0x00000000);
    ...
    r = 0x00010013;
    dbg_printf("Write DDR_PHY_CONTROL_REGS_VDL_CONTROL_CKE(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_VDL_CONTROL_CKE, r);
    ...
    r = 0x00010013;
    dbg_printf("Write DDR_PHY_CONTROL_REGS_VDL_CONTROL_PAR(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_VDL_CONTROL_PAR, r);
   ...
   dbg_printf("---Perform VDL Override---\n");
    ...
    dbg_printf("----- cal_steps(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_VDL_CALIB_STATUS1, r);
    ...
    for (bl = 0; bl < maxbl; bl++) {
        ...
        r = 0x10100;
        dbg_printf("Write DDR_PHY_BYTE_LANE_%d_RD_EN_DLY_CYC(0x%x) = 0x%x\n", 
                   bl, (DDR_PHY_BYTE_LANE_0_RD_EN_DLY_CYC + 0x200 * bl), r);
        reg32_write((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_RD_EN_DLY_CYC + 0x200 * bl), r);
//        r = 0x10018;
        r = 0x100FF;
        dbg_printf("ReadAfterWrite DDR_PHY_BYTE_LANE_%d_VDL_CONTROL_RD_EN_CS0(0x%x) = 0x%x\n",
                    bl, (DDR_PHY_BYTE_LANE_0_VDL_CONTROL_RD_EN_CS0 + 0x200 * bl), r);
        reg32_write((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_VDL_CONTROL_RD_EN_CS0 + 0x200 * bl), r);     
//        r = 0x10018;
        r = 0x100FF;
        dbg_printf("ReadAfterWrite DDR_PHY_BYTE_LANE_%d_VDL_CONTROL_RD_EN_CS1(0x%x) = 0x%x\n", 
                    bl, (DDR_PHY_BYTE_LANE_0_VDL_CONTROL_RD_EN_CS1 + 0x200 * bl), r);
        reg32_write((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_VDL_CONTROL_RD_EN_CS1 + 0x200 * bl), r);
    }
    /* End of VDL Override  */
    ...
    r = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_DRIVE_PAD_CTL );
    dbg_printf("ReadAfterWrite DDR_PHY_CONTROL_REGS_DRIVE_PAD_CTL(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_DRIVE_PAD_CTL, r);

    /* 0x00031bff = (31 | (31 << 5) | (6 <<10) | (6 << 15)) */
    for (bl = 0; bl < maxbl; bl++) {
        reg32_write((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_DRIVE_PAD_CTL + 0x200 * bl), 0x00031bff);
        r = reg32_read((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_DRIVE_PAD_CTL + 0x200 * bl));
        dbg_printf("ReadAfterWrite DDR_PHY_BYTE_LANE_%d_DRIVE_PAD_CTL(0x%x) = 0x%x\n", 
                    bl, (DDR_PHY_BYTE_LANE_0_DRIVE_PAD_CTL + 0x200 * bl), r);
    }

    
    for (bl = 0; bl < maxbl; bl++) {
        reg32_write((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_DQSP_DRIVE_PAD_CTL + 0x200 * bl), 0x00031bff);
        r = reg32_read((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_DQSP_DRIVE_PAD_CTL + 0x200 * bl));
        dbg_printf("ReadAfterWrite DDR_PHY_BYTE_LANE_%d_DQSP_DRIVE_PAD_CTL(0x%x) = 0x%x\n", 
                    bl, (DDR_PHY_BYTE_LANE_0_DQSP_DRIVE_PAD_CTL + 0x200 * bl), r);

        reg32_write((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_DQSN_DRIVE_PAD_CTL + 0x200 * bl), 0x00031bff);
        r = reg32_read((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_DQSN_DRIVE_PAD_CTL + 0x200 * bl));
        dbg_printf("ReadAfterWrite DDR_PHY_BYTE_LANE_%d_DQSN_DRIVE_PAD_CTL(0x%x) = 0x%x\n", 
                    bl, (DDR_PHY_BYTE_LANE_0_DQSN_DRIVE_PAD_CTL + 0x200 * bl), r);

        reg32_write((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_RD_EN_DRIVE_PAD_CTL + 0x200 * bl),
                   31 | (31 << 5) | (31 <<10) | (31 << 15));
        r = reg32_read((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_RD_EN_DRIVE_PAD_CTL + 0x200 * bl));
        dbg_printf("ReadAfterWrite DDR_PHY_BYTE_LANE_%d_RD_EN_DRIVE_PAD_CTL(0x%x) = 0x%x\n", 
                    bl, (DDR_PHY_BYTE_LANE_0_RD_EN_DRIVE_PAD_CTL + 0x200 * bl), r);
    }
    ...
    val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STATIC_PAD_CTL);
    dbg_printf("Read DDR_PHY_CONTROL_REGS_STATIC_PAD_CTL Val(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_STATIC_PAD_CTL, val);
    ...
    */
    ...
    for (bl = 0; bl < maxbl; bl++) {
        ...
        r = reg32_read((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_WR_PREAMBLE_MODE + 0x200 * bl));
        dbg_printf("ReadAfterWrite DDR_PHY_BYTE_LANE_%d_WR_PREAMBLE_MODE(0x%x) = 0x%x\n", 
                    bl, (DDR_PHY_BYTE_LANE_0_WR_PREAMBLE_MODE + 0x200 * bl), r);
            
        reg32_write_masked((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_STATIC_PAD_CTL + 0x200 * bl), 0x00020000, 0x00000);
        r = reg32_read((volatile uint32_t *)(DDR_PHY_BYTE_LANE_0_STATIC_PAD_CTL + 0x200 * bl));
        dbg_printf("ReadAfterWrite DDR_PHY_BYTE_LANE_%d_STATIC_PAD_CTL(0x%x) = 0x%x\n", 
                    bl, (DDR_PHY_BYTE_LANE_0_STATIC_PAD_CTL + 0x200 * bl), r);
    }
    ...
    val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_ABI_PAR_CNTRL);
    dbg_printf("ReadAfterWrite DDR_PHY_CONTROL_REGS_ABI_PAR_CNTRL(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_ABI_PAR_CNTRL, val);
    ...
    */
    ...
    val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_DFI_CNTRL);
    dbg_printf("ReadAfterWrite DDR_PHY_CONTROL_REGS_DFI_CNTRL(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_DFI_CNTRL, val);
    ...
    val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STANDBY_CONTROL);
    dbg_printf("ReadAfterWrite DDR_PHY_CONTROL_REGS_STANDBY_CONTROL(0x%x) = 0x%x\n", DDR_PHY_CONTROL_REGS_STANDBY_CONTROL, val);
    /* End of PHY misc init */

    if (ddr_type == 0) {  /* DDR3 */
 	if (ddr_clk == 400)
    	{
    	   printf("Enabling ddr3_init_tab_800\n");
    	   ddr_init_regs(ddr3_init_tab_800);
        } else if (ddr_clk == 533)
        {
       	   printf("Enabling ddr3_init_tab_1066\n");
      	   ddr_init_regs(ddr3_init_tab_1066);
        } else if (ddr_clk == 667)
        {
    	...
    	{
     	   printf("Enabling ddr3_init_tab_1600\n");
    	   ddr_init_regs(ddr3_init_tab_1600);
    	}
    } else {                 /* DDR4 */
        printf("Enabling mt40a512m8_093_1600\n");
    ...
    /* Start the DDR */
        reg32_set_bits((volatile uint32_t *)DDR_DENALI_CTL_00, 0x01);
    val = reg32_read((volatile uint32_t *)DDR_DENALI_CTL_00);
    dbg_printf("ReadAfterWrite DDR_DENALI_CTL_00(0x%x) = 0x%x\n", DDR_DENALI_CTL_00, val);

#if defined(CONFIG_IPROC_P7) && defined(CONFIG_IPROC_DDR_ECC)
    printf("Enabling DDR ECC correcting and reporting\n");
    
    /* Clear DDR ECC interrupts if any */
    reg32_set_bits((volatile uint32_t *)DDR_DENALI_CTL_177, DDR_DENALI_CTL_177_ECC_MASK);

    /* Disable auto corruption */
    reg32_set_bits((volatile uint32_t *)DDR_DENALI_CTL_148, 1 << DDR_DENALI_CTL_148__ECC_DISABLE_W_UC_ERR);

    /* Enable ECC correction and reporting */
    reg32_set_bits((volatile uint32_t *)DDR_DENALI_CTL_146, 1 << DDR_DENALI_CTL_146__ECC_EN);
...
#if defined(CONFIG_GREYHOUND2)
    while (*(volatile unsigned int *)DDR_PHY_CONTROL_REGS_REVISION == 0);   // DDR_PHY_CONTROL_REGS_REVISION
#else    
