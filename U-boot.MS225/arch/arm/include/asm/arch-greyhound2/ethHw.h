...
int ethHw_Init(int pidx);
int ethHw_Exit(void);

int ethHw_macEnableSet(int pidx, int en);
int ethHw_macEnableGet(int pidx, int *txp, int *rxp);
int ethHw_macLoopbackSet(int pidx, int en);

int ethHw_dmaTx(int pidx, uint8_t *tx_buf, size_t len);
int ethHw_dmaTxWait(int pidx);
int ethHw_dmaRx(int pidx, uint8_t *buf, size_t *len);
int ethHw_dmaDisable(int pidx, int dir);
int ethHw_dmaEnable(int pidx, int dir);
int ethHw_dmaLoopbackSet(int pidx, int en);

int ethHw_portLinkUp(int pidx);
void ethHw_portSpeedCheck(int pidx);

int ethHw_miiphy_read(uint32_t const phyaddr, uint32_t const reg, uint16_t *const value);
int ethHw_miiphy_write(uint32_t const phyaddr, uint32_t const reg, uint16_t *const value);

void ethHw_dump(int pidx, int type);
