...
static void pl310_background_op_all_ways(u32 *op_reg)
{
	...
	assoc_16 = cpu_to_le32(readl(&pl310->pl310_aux_ctrl)) &
	...
	/* Invalidate all ways */
	writel(cpu_to_le32(way_mask), op_reg);
	/* Wait for all ways to be invalidated */
	while (cpu_to_le32(readl(op_reg)) && way_mask)
		;
	pl310_cache_sync();
}
...
void v7_outer_cache_flush_range(u32 start, u32 stop)
{
	...
	for (pa = start; pa < stop; pa = pa + line_size)
		writel(cpu_to_le32(pa), &pl310->pl310_clean_inv_line_pa);
...
void v7_outer_cache_inval_range(u32 start, u32 stop)
{
	...
	for (pa = start; pa < stop; pa = pa + line_size)
		writel(cpu_to_le32(pa), &pl310->pl310_inv_line_pa);
...
{
	writel(cpu_to_le32(1), &pl310->pl310_ctrl);
}
